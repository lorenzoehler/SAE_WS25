...
#### Bootstrap vs Analytic Direct Estimation

```{r}
# direct_n50 <- emdi::direct(y = "aestudio", smp_data = sample ,smp_domains = "ID_prov",var = T,na.rm = T,B = 50)
...
tmp_dat <- merge(direct_n500$MSE[,c("Domain","Mean")],direct_estimates[,c("ID_prov","Var_Mean")],by.x = "Domain", by.y = "ID_prov")
plot(tmp_dat$Mean,tmp_dat$Var_Mean)
...
print(fh_model_full_unoptimized)
# (sqrt(fh_model_full$MSE$FH)/fh_model_full$ind$FH * 100) %>% mean()
# fh_model_full_unoptimized %>% summary()
```
### FH Step
...
BHF_model_full_unoptimized %>% summary(())
```
...
plot_grid_fh <- plot_grid(p2,p4,p6,p10,nrow = 4)
plot_grid_fh
...
```{r fig.width=6, fig.height=16}
plot_grid_bhf <- plot_grid(p2,p4,p6,p10,nrow = 4)
plot_grid_bhf

...
head1000_EH_person <- read_sav("../../data_raw/2021_houshold-survey/EH2021_Persona.sav")

var_labels <- sapply(head1000_EH_person, attr, "label")
var_labels %>% unlist()
dat_EV <- read_sav("../../../../data_raw/data_raw/2021_houshold-survey/EH2021_Persona.sav")

head1000_EH_person

var_info <- data.frame(
  variable = names(head1000_EH_person),
  label = sapply(head1000_EH_person, function(x) attr(x, "label")),
  row.names = NULL
)
...
```{r}
dat_housholdsuvery_person <- read_sav("../../data_raw/2021_houshold-survey/EH2021_Persona.sav")

dat_EV <- read_sav(
  "/Users/lorenzoehler/Documents/GitHub/SAE_WS25/../../data_raw/data_raw/2021_houshold-survey/EH2021_Persona.sav",
  encoding = "latin1"
)

lapply(dat_EV,FUN = summary)


dat_housholdsurvey_person$upm
```

how many pers houshold per area?

```{r}
dat_housholdsuvery_person %>% 

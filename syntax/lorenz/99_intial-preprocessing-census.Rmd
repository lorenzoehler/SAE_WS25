...
dat_person_raw$p26_edad <- dat_person_raw$p26_edad %>% as.numeric()

dat_person_raw %>% filter(p26_edad > 18) %>% 
  pull(aestudio) %>% 
  hist(., main = "Anzahl der Schuljahr für Personen über 18")

dat <- dat_person_raw %>% filter(p26_edad > 18) %>%  group_by(ADM3_PCODE) %>% summarise(mean_years_of_education = mean(aestudio, na.rm = T))
```
...
nrow(dat_housholdsurvey_person)
```

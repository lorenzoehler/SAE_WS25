...
### only use people older then 18
(dat_person_raw$p26_edad < 18) %>% sum()
dat_person_raw <- dat_person_raw %>% filter(p26_edad >= 18) 

### only use people with aestudio
dat_person_raw$aestudio %>% is.na() %>% sum()
dat_person_raw$aestudio %>% is.na() %>% mean()
dat_person_raw <- dat_person_raw[!is.na(dat_person_raw$aestudio)]
...
sample[,c("idep","hh_idep","iprov","hh_iprov","imun")]

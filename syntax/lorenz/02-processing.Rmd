...
mapping_sheet <- tmp_sheet %>% mutate(label = paste0(prefix, label)) %>% select(value, label)

column <- sample[[sheets_selection[i]]]
...
### first colums have †o be transformed
size <- nrow(dat)
dummy_coding <- data.frame(matrix(nrow = size,ncol = 0))
...
saveRDS(true_mean,"../../data_raw/misc/true_mean_aestudio.RDS")
```
